...
const report = {
  ...
  methodology: { warmups, iterations, sizes, timer: 'performance.now', p95: 'nearest rank',
    scope: 'Synchronous TypeScript removeBackground call, including its allocations and incidental GC. Decoding, cropping, resizing, PNG encoding, file I/O, worker startup and transfer excluded. Sequential calls, no forced GC.' },
  results,
};
...
await updateReadme('benchmark', [
  ...
  '', `${warmups} warm-up calls and ${iterations} timed calls per row. Times cover the synchronous TypeScript algorithm and its allocations; they exclude decoding, cropping, resizing, PNG encoding, file I/O, and worker overhead. p95 uses nearest rank. Inputs, crops, and options match the showcase cases; images are never enlarged.`,
